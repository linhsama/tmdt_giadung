...
$donHang__Get_By_Id_Kh = $dh->DonHang__Get_By_Id_KH($id_kh);

$id_don = isset($_POST['id_don']) ? $_POST['id_don'] : 0;
                                                            ...
                                                            <tbody>
                                                                <?php foreach ($donHang__Get_By_Id_Kh as $item) : ?>
                                                                    <tr>
                                                                        ...
                                                                        <td><?= isset($cttt->ChiTietTrangThai__Get_Last_By_DH($item->id_don)->tentt) ?  $cttt->ChiTietTrangThai__Get_Last_By_DH($item->id_don)->tentt : 'Chưa xác nhận!' ?></td>
                                                                        <td><?= isset($cttt->ChiTietTrangThai__Get_Last_By_DH($item->id_don)->ngaytao) ?  $cttt->ChiTietTrangThai__Get_Last_By_DH($item->id_don)->ngaytao : 'Chưa xác nhận' ?></td>
                                                                        <td class="text-center font-weight-bold">
                                                                            <?php if (
                                                                                $cttt->ChiTietTrangThai__Check($item->id_don, 1) != false // đơn bị từ chối bởi người bán
                                                                            ) : ?>
                                                                                <button type="button" class="btn btn-success btn-secondary" onclick="return view('<?= $item->id_don ?>')">
                                                                                    <i class="bx bx-edit" aria-hidden="true"></i> Đơn đã hủy
                                                                                </button>

                                                                            <?php elseif ($cttt->ChiTietTrangThai__Check($item->id_don, 6) != false) :   // đơn được giao thành công)
                                                                            ?>

                                                                                <button type="button" class="btn btn-sm btn-success btn-update" onclick="return view('<?= $item->id_don ?>')">
                                                                                    <i class="bx bxs-happy-heart-eyes" aria-hidden="true"></i> Đơn đã giao
                                                                                </button>
                                                                            <?php else : ?>
                                                                                <?php if (!isset($cttt->ChiTietTrangThai__Get_Last_By_DH($item->id_don)->tentt)) : ?>
                                                                                    <button type="button" class="btn btn-sm btn-danger btn-update" onclick="return remove('<?= $item->id_don ?>')">
                                                                                        <i class="bx bxs-edit" aria-hidden="true"></i> Hủy đơn
                                                                                    </button>
                                                                                    
                                                                                <button type="button" class="btn btn-sm btn-primary btn-update" onclick="return view('<?= $item->id_don ?>')">
