                        ...
                        <tbody>
                            <?php foreach ($donHang__Get_All as $item) : ?>
                                <tr>
                                    <td><?= $item->id_don ?></td>
                                    <td><?= $item->ngaythem ?></td>
                                    <td><?= $kh->KhachHang__Get_By_Id($item->id_kh)->tenkh ?></td>
                                    ...
                                    <td class="text-center font-weight-bold">
                                        <?php if (
                                            $cttt->ChiTietTrangThai__Check($item->id_don, 1) != false // đơn bị hủy bởi người bán
                                        ) : ?>
                                            <button type="button" class="btn btn-success btn-secondary" onclick="return update_obj('<?= $item->id_don ?>')">
                                        ...
                                        ?>
                                            <button type="button" class="btn btn-success btn-update" onclick="return update_obj('<?= $item->id_don ?>')">
                                                <i class="bx bx-edit" aria-hidden="true"></i> Đơn đã giao
                                            </button>
                                        <?php else : ?>
                                            <button type="button" class="btn btn-danger btn-update" onclick="return update_obj('<?= $item->id_don ?>')">
    ...
    function delete_obj(id_don) {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                location.href = "pages/don-hang/action.php?req=delete&madon=" + id_don;
            } else if (
