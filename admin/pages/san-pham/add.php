    ...
    <form class="form-group" action="pages/san-pham/action.php?req=add" method="post" enctype="multipart/form-data">
        ...
        <div class="col">
            ...
            <div class="form-check form-check-inline">
                <input class="form-check-input checkbox" type="radio" id="id_th<?= $item->id_th ?>"
                    value="<?= $item->id_th ?>" name="id_th" required>
        ...
        <div class="col">
            ...
            <div class="form-check form-check-inline">
                <input class="form-check-input checkbox id_loai_checkbox" type="radio" id="id_loai<?= $item->id_loai ?>"  onclick="show_thuoctinh('<?=$item->id_loai?>', '<?= $item->tenloai ?>')"
                    value="<?= $item->id_loai ?>" name="id_loai" required>
                <label class="form-check-label" for="id_loai<?= $item->id_loai ?>"><?= $item->tenloai ?></label>
            </div>
            <?php endforeach; ?>
        </div>
        <div class="col">
            <div class="form-thuoctinh"></div>
        </div>
        <div class="col">
...
window.addEventListener('load', function (){
    document.querySelector('.id_loai_checkbox').checked = true;
    show_thuoctinh('<?=$loaiSp__Get_All[0]->id_loai?>', '<?= $loaiSp__Get_All[0]->tenloai ?>')

});


function show_thuoctinh(id_loai, tenloai){
    $.post("pages/san-pham/thuoctinh.php", {
        id_loai: id_loai, 
        tenloai: tenloai
    }, function(data, status) {
        $(".form-thuoctinh").html(data);
    });
}
